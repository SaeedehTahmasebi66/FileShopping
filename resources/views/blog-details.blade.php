						...
						<div class="blog-details content">
							<article class="blog-post-details">
								<div class="post-thumbnail">
                                    @foreach($blog->Image()->get() as $Img)
									<img src="{{asset('/') . $Img->path}}" alt="blog images">
                                    @endforeach
                                </div>
							...
							<div class="comments_area">
                                ...
                                <div>
                                    <h6 class="pb--30"> {{$blog->Comment()->get()->count()}} نفر راجع به این مطلب دیدگاه خود را ثبت کرده اند:</h6>
                                    @foreach($blog->Comment()->get() as $C)
                            ...
                            @endif
                            <div class="comment_respond">
                                <h6 class="ptb--30">نظر شما در مورد این مطلب:</h6>
                                <form class="comment__form" action="{{'/insertComment'}}" method="post">
                                    {{ csrf_field() }}
                                    <input type="hidden" class="form-control " name="id" value="{{$blog['id']}}" >
                                    <input type="hidden" class="form-control " name="type" value="{{'App\Models\Blog'}}" >
                                <div class="input__box">
                                    <textarea name="review" placeholder="دیدگاه خود را اینجا ثبت کنید..."></textarea>
                                </div>
                                <div class="submite__btn">
